...
- **Synchronous:** Services communicate via internal HTTP APIs. Each service boundary passes through and validates auth headers.
- **Asynchronous:** Workers consume from the job queue. The Core API publishes jobs; workers process them independently.
- **Events:** The Core API publishes significant domain events (user created, subscription changed) to the event bus for downstream consumers.
